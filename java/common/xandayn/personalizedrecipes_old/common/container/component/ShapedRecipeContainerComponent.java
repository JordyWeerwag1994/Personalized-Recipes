...
/**
 * PersonalizedRecipes - ShapedRecipeContainerComponent
 *
 * A class that defines a shaped component for the RecipeCreatorGui.
 *
 * @license
 ...
 * @author xandayn
 */
public class ShapedRecipeContainerComponent implements IRecipeContainerComponent {
    ...
    public IRecipe compileRecipe(ItemStack[] data) {
        ...
        ItemStack output = data[9].copy();
        return new ShapedRecipes(3, 3, compiledRecipe.toArray(new ItemStack[compiledRecipe.size()]), output);
    }
    ...
    public void addRecipeToCraftingWindow(IRecipe recipe, ItemStack[] table) {
        if(recipe instanceof ShapedRecipes){
            ShapedRecipes realRecipe = (ShapedRecipes)recipe;
            for(int i = 0; i < 9; i++){
                table[i] = realRecipe.recipeItems[i];
            }
            table[9] = realRecipe.getRecipeOutput();
        }
    }
